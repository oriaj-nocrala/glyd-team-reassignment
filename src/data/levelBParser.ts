      ...
      fs.createReadStream(filePath)
        ...
        .on('data', (row: EventCSVRow) => {
          try {
            ...
            logger.warn(`Warning: Failed to parse event row ${JSON.stringify(row)}: ${error}`);
          }
        })
        .on('end', () => {
      ...
      fs.createReadStream(filePath)
        ...
        .on('data', (row: MessageCSVRow) => {
          try {
            ...
            logger.warn(`Warning: Failed to parse message row ${JSON.stringify(row)}: ${error}`);
          }
        })
        .on('end', () => {
      ...
      fs.createReadStream(filePath)
        ...
        .on('data', (row: SpendCSVRow) => {
          try {
            ...
            logger.warn(`Warning: Failed to parse spend row ${JSON.stringify(row)}: ${error}`);
          }
        })
        .on('end', () => {
