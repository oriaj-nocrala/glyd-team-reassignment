...
export class MetricsCalculator {
  ...
  static calculateTeamBalance(teams: { players: PlayerWithScore[] }[]): {
    ...
    const teamMeans = teams.map((team) => {
      ...
      return (
        team.players.reduce((sum, player) => sum + player.composite_score, 0) / team.players.length
      );
    });

    const overallMean = teamMeans.reduce((sum, mean) => sum + mean, 0) / teamMeans.length;
    const variance =
      teamMeans.reduce((sum, mean) => sum + Math.pow(mean - overallMean, 2), 0) / teamMeans.length;
   ...
   */
  static calculateTeamStats(team: { players: PlayerWithScore[] }): {
    total_score: number;
    average_score: number;
    size: number;
  } {
    if (team.players.length === 0) {
      return { total_score: 0, average_score: 0, size: 0 };
    }

    const total_score = team.players.reduce((sum, player) => sum + player.composite_score, 0);
    const average_score = total_score / team.players.length;
