...
export class OutputFormatter {
  ...
  private static formatTeamDetails(teams: Team[]): string {
    ...
    teams.forEach(team => {
      ...
      sortedPlayers.forEach((player, index) => {
        ...
        const oldTeamInfo = player.current_team_name !== '' ? ` (was: ${player.current_team_name})` : '';
  ...
  private static formatFairnessReport(result: AssignmentResult): string {
    ...
    // Overall assessment
    const balanceGrade = this.assessBalance(fairness_stats.score_standard_deviation, size_balance.size_difference);
  ...
  static formatChangesSummary(result: AssignmentResult): string {
    ...
    if (moves.length > 0) {
      ...
      moves.sort((a, b) => a.player_id - b.player_id).forEach(move => {
